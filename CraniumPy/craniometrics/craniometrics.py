...
class CranioMetrics:
    ...
    def slice_mesh(self, slice_d):
        ...
        # collect slice bounds (DataFrame.append removed in pandas 2.0)
        slice_rows = []
        ...
        self.slice_df = pd.DataFrame(slice_rows)

        if len(self.slice_df) == 0:
            raise ValueError("No slices found; mesh may have wrong orientation or bounds")
        ...
        count_b = 0
        while (self.slice_index < len(self.slice_df) and
               self.slice_df.breadth.iloc[self.slice_index] >= 180 and
               count_b <= 100):
            count_b += 1
            self.slice_index += self.slice_d

        self.slice_index = min(self.slice_index, len(self.slice_df) - 1)
    ...
    def extract_dimensions(self, slice_height):
        ...
        HCP = HC_slice.points
        if len(HCP) < 2:
            raise ValueError("Slice has insufficient points; mesh may need registration first")
        polar = []
        ...
        # Use argmin for robust lookup (exact float equality often fails)
        lh_opt_index = np.argmin(np.abs(self.HC_s.points[:, 0] - hb[0]))
        self.lh_opt = self.HC_s.points[lh_opt_index]

        rh_opt_index = np.argmin(np.abs(self.HC_s.points[:, 0] - hb[1]))
        self.rh_opt = self.HC_s.points[rh_opt_index]

        occ_opt_index = np.argmin(np.abs(self.HC_s.points[:, 2] - hb[4]))
        self.occ_opt = self.HC_s.points[occ_opt_index]

        front_opt_index = np.argmin(np.abs(self.HC_s.points[:, 2] - hb[5]))
        self.front_opt = self.HC_s.points[front_opt_index]

        self.optima_df = pd.DataFrame([{
